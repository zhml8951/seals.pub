#!/usr/bin/env bash
#
# eseals jboss auto install script in CentOS7/Redhat7/Oracle7 
...
hostsCheck() {
	local netInfo=''
    local ipUsed=''
	if netInfo=$(route -n | grep '^0\.0.*'); then
		true
	else
	...
	# network information 
    local adapterUsed=$(echo $netInfo | awk '{print $NF}')
	local defNet=`echo $netInfo | awk '{print $2}' | cut -d'.' -f '1-3'`
	local ipUsed=$(ip addr show | grep "$defNet" | grep "$adapterUsed" | awk '{print $2}' | cut -d'/' -f1)
	
	# host name get and set 
	local hostName=`hostname`
    to_hosts="echo $ipUsed $hostName"
    if grep '^127.*' /etc/hosts >/dev/null 2>&1 && grep '^::.*' /etc/hosts >/dev/null 2>&1; then 
